...
class StartupSyncService @Inject constructor(
    ...
    private suspend fun pullRemoteData(
        ...
        try {
            ...
            if (!authManager.supportsFullCloudSync) {
                pullPanelManagedAddonsAndPlugins(profileId)
    ...
    private suspend fun pullPanelManagedAddonsAndPlugins(profileId: Int) = coroutineScope {
        val pluginJob = async {
            pluginManager.isSyncingFromRemote = true
            try {
                val remotePlugins = pluginSyncService.getRemoteRepoUrls().getOrElse { throw it }
                ...
                Log.d(TAG, "Pulled ${remotePlugins.size} panel-managed plugin repos for profile $profileId")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to pull panel-managed plugins, keeping local cache", e)
            } finally {
                pluginManager.isSyncingFromRemote = false
                pluginManager.flushPendingSync()
            }
        ...
        val addonJob = async {
            ...
            try {
                ...
                Log.e(TAG, "Failed to pull panel-managed addons, keeping local cache", e)
            } finally {
                addonRepository.isSyncingFromRemote = false
            }
        }
